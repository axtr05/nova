...
export function Sidebar({
  ...
  const completedTasksToday = todayEvents.filter((evt) =>
    completedEventIds.includes(evt.id)
  ).length;
        ...
        <div className="flex-1 overflow-y-auto pr-1 flex flex-col gap-3 min-h-0">
          <AnimatePresence initial={false}>
            {todayEvents.length === 0 ? (
              ...
              todayEvents.map((evt) => {
                const isCompleted = completedEventIds.includes(evt.id);
                ...
                return (
                  <motion.div
                    ...
                    onClick={() => toggleCompleteEvent(evt.id)}
